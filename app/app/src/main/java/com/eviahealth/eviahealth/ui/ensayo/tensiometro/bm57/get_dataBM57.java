...
public class get_dataBM57 extends BaseActivity implements View.OnClickListener {

    private String TAG = "GET_DATA_BM57";
    private String FASE = "GET_DATA_BM57";
    ...
    private bm57control mBM57control = new bm57control();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_get_data_bm57);

        //region :: Views
        txtStatus = findViewById(R.id.txtStatus_bm57);
        txtStatus.setTextSize(14);
        txtStatus.setVisibility(View.VISIBLE);
        circulodescarga = findViewById(R.id.cdescarga_bm57);
        circulodescarga.setVisibility(View.INVISIBLE);

        btSaltar = findViewById(R.id.btSaltar_bm57);
        ...
        if (DEVICE_MAC_ADDRESS.contains("BM57")){
            DEVICE_MAC_ADDRESS = DEVICE_MAC_ADDRESS.replace("BM57-", "");
...
//                        btSaltar.setVisibility(View.VISIBLE);
                        ...
                        mBM57control.initialize(get_dataBM57.this,mDeviceAddress);
                    }
                });
            }
        };
    ...
    public void onClick(View view) {
        ...
        if (viewId == R.id.btSaltar_bm57) {
            ...
            String frase = "Coloque el brazalete en el brazo descubierto, de forma que el borde quede entre 2-3 centímetros por encima del codo y sobre la arteria.";
            texto.add(frase);
            frase = "Mantenga un espacio de 2 dedos entre su brazo y el brazalete.";
            texto.add(frase);
            frase = "Coloque el brazalete en posición que el aparato esté a la altura del corazón durante la medición.";
            texto.add(frase);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch(action) {
                ...
                case BeurerReferences.ACTION_BEURER_DISCONNECTED:
                    ...
                    if (isconnect == Global.STATE_CONNECTING) {
                        if (retries < 10) {
                            ...
                            mBM57control.destroy();
                            SystemClock.sleep(1000);
                            mBM57control.initialize(get_dataBM57.this, mDeviceAddress);
                        }
                        else {
                            datos.set_status_descarga(false);
                            datos.setError("{\"type\":\"BM57\",\"error\":801,\"description\":\"No detectado dispositivo.\"}");
                            ViewResult();
                        }
                    }
                    else if (isconnect == Global.STATE_CONNECTED) {
                        // EL DISPOSITIVO SE HA DESCONECTADO SOLO
                        message = mBM57control.getRecordData();
                        ...
                        if (status == 0 || status == 4 ) {
                            ...
                            if (status == null) {
                                String error = "{\"type\":\"BM57\",\"error\":-1,\"description\":\"No detectado dispositivo.\"}";
                                broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                            } else {
                                String error = mBM57control.msgGetMeasurementStatus(message);
                                Log.e(TAG, "Problems: " + error);

                                if (util.getIntValueJSON(error, "bodyMovement") == 1) {
                                    error = "{\"type\":\"BM57\",\"error\":1,\"description\":\"Movimiento del cuerpo.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "cuffFit") == 1) {
                                    error = "{\"type\":\"BM57\",\"error\":2,\"description\":\"El manguito está demasiado suelto.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "measurementPosition") == 1) {
                                    error = "{\"type\":\"BM57\",\"error\":6,\"description\":\"Detectada posición incorrecta del dispositivo.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 1) {
                                    error = "{\"type\":\"BM57\",\"error\":3,\"description\":\"El pulso detectado excede el limite superior.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 2) {
                                    error = "{\"type\":\"BM57\",\"error\":4,\"description\":\"El pulso detectado se encuentra por debajo del limite inferior.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 3) {
                                    error = "{\"type\":\"BM57\",\"error\":5,\"description\":\"Detectado un problema con la medición del pulso.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else {
                                    error = "{\"type\":\"BM57\",\"error\":-1,\"description\":\"not found.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                            }
                        }
                ...
                case BeurerReferences.ACTION_BEURER_BC54_DOWNLOAD_DATA:
                    Log.e(TAG,"ACTION_BEURER_BM57_DOWNLOAD_DATA");
                    setTextSatus("Download");
                    break;

                case BeurerReferences.ACTION_BEURER_BC54_DOWNLOAD_DATA_FINISHED:
                    Log.e(TAG,"ACTION_BEURER_BM57_DOWNLOAD_DATA_FINISHED Message:" + message);
                ...
                case BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR:
                    Log.e(TAG,"ACTION_BEURER_BM57_MEASUREMENT_ERROR Message:" + message);
                    datos.set_status_descarga(false);
                ...
                case BeurerReferences.ACTION_BEURER_CMD_FAIL:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BM57\",\"error\":803,\"description\":\"Detectado un fallo de comunicación con el dispositivo..\"}");

                    mBM57control.disconnect();
                    break;
                ...
                case BeurerReferences.ACTION_BEURER_COMMUNICATION_TIMEOUT:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BM57\",\"error\":802,\"description\":\"Ha fallado la descarga de datos con el dispositivo.\"}");
                    mBM57control.disconnect();
                    break;
                ...
                default:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BM57\",\"error\":804,\"description\":\"ACCION NO CONTEMPLADA\"}");
                    mBM57control.disconnect();
                    break;
    ...
    public void onDestroy() {
        ...
        if (mBM57control != null) { mBM57control.destroy(); }
        mBM57control = null;
    ...
    public void ViewResult() {
        if (viewdata == false) {
            ...
            if (datos.get_status_descarga() == false) {
                Intent in = new Intent(this, view_failBM57.class);
                startActivity(in);
            }
            else {
                Intent in = new Intent(this, view_dataBM57.class);
                startActivity(in);
            }
