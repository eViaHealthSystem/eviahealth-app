package com.eviahealth.eviahealth.ui.ensayo.tensiometro.bc85;
...
import com.eviahealth.eviahealth.models.beurer.bc85.bc85DataTensiometro;
import com.eviahealth.eviahealth.bluetooth.beurer.bc85.bc85control;
...
public class get_dataBC85 extends BaseActivity implements View.OnClickListener {

    private String TAG = "GET_DATA_BC85";
    private String FASE = "GET_DATA_BC85";
    ...
    private bc85control mBC85control = new bc85control();
    bc85DataTensiometro datos;
    private int retries;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_get_data_bc85);

        //region :: Views
        txtStatus = findViewById(R.id.txtStatus_bc85);
        txtStatus.setTextSize(14);
        txtStatus.setVisibility(View.VISIBLE);
        circulodescarga = findViewById(R.id.cdescarga_bc85);
        circulodescarga.setVisibility(View.INVISIBLE);

        btSaltar = findViewById(R.id.btSaltar_bc85);
        ...
        if (DEVICE_MAC_ADDRESS.contains("BC85")){
            DEVICE_MAC_ADDRESS = DEVICE_MAC_ADDRESS.replace("BC85-", "");
            mDeviceAddress = util.MontarMAC(DEVICE_MAC_ADDRESS);
            EVLog.log(TAG,"MAC mDeviceAddress: "+ mDeviceAddress);
        }

        datos = bc85DataTensiometro.getInstance();
        datos.clear();
        retries = 0;
...
//                        btSaltar.setVisibility(View.VISIBLE);
                        ...
                        mBC85control.initialize(get_dataBC85.this,mDeviceAddress);
                    }
                });
            }
        };
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch(action) {
                ...
                case BeurerReferences.ACTION_BEURER_DISCONNECTED:
                    ...
                    if (isconnect == Global.STATE_CONNECTING) {
                        if (retries < 10) {
                            ...
                            mBC85control.destroy();
                            SystemClock.sleep(1000);
                            mBC85control.initialize(get_dataBC85.this, mDeviceAddress);
                        }
                        else {
                            datos.set_status_descarga(false);
                            datos.setError("{\"type\":\"BC85\",\"error\":801,\"description\":\"No detectado dispositivo.\"}");
                            ViewResult();
                        }
                    }
                    else if (isconnect == Global.STATE_CONNECTED) {
                        // EL DISPOSITIVO SE HA DESCONECTADO SOLO
                        message = mBC85control.getRecordData();
                        ...
                        if (status == 0 ||status == 4 ) {
                            ...
                            if (status == null) {
                                String error = "{\"type\":\"BC85\",\"error\":-1,\"description\":\"No detectado dispositivo.\"}";
                                broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                            } else {
                                String error = mBC85control.msgGetMeasurementStatus(message);
                                Log.e(TAG, "Problems: " + error);

                                if (util.getIntValueJSON(error, "bodyMovement") == 1) {
                                    error = "{\"type\":\"BC85\",\"error\":1,\"description\":\"Movimiento del cuerpo.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "cuffFit") == 1) {
                                    error = "{\"type\":\"BC85\",\"error\":2,\"description\":\"El manguito está demasiado suelto.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "measurementPosition") == 1) {
                                    error = "{\"type\":\"BC85\",\"error\":6,\"description\":\"Detectada posición incorrecta del dispositivo.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 1) {
                                    error = "{\"type\":\"BC85\",\"error\":3,\"description\":\"El pulso detectado excede el limite superior.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 2) {
                                    error = "{\"type\":\"BC85\",\"error\":4,\"description\":\"El pulso detectado se encuentra por debajo del limite inferior.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 3) {
                                    error = "{\"type\":\"BC85\",\"error\":5,\"description\":\"Detectado un problema con la medición del pulso.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else {
                                    error = "{\"type\":\"BC85\",\"error\":-1,\"description\":\"not found.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                            }
                        }
                ...
                case BeurerReferences.ACTION_BEURER_BC54_DOWNLOAD_DATA:
                    Log.e(TAG,"ACTION_BEURER_BC85_DOWNLOAD_DATA");
                    setTextSatus("Download");
                    break;

                case BeurerReferences.ACTION_BEURER_BC54_DOWNLOAD_DATA_FINISHED:
                    Log.e(TAG,"ACTION_BEURER_BC85_DOWNLOAD_DATA_FINISHED Message:" + message);
                ...
                case BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR:
                    Log.e(TAG,"ACTION_BEURER_BC85_MEASUREMENT_ERROR Message:" + message);
                    datos.set_status_descarga(false);
                ...
                case BeurerReferences.ACTION_BEURER_CMD_FAIL:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BC85\",\"error\":803,\"description\":\"Detectado un fallo de comunicación con el dispositivo..\"}");

                    mBC85control.disconnect();
                    break;
                ...
                case BeurerReferences.ACTION_BEURER_COMMUNICATION_TIMEOUT:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BC85\",\"error\":802,\"description\":\"Ha fallado la descarga de datos con el dispositivo.\"}");
                    mBC85control.disconnect();
                    break;
                ...
                default:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BC85\",\"error\":804,\"description\":\"ACCION NO CONTEMPLADA\"}");
                    mBC85control.disconnect();
                    break;
    ...
    public void onDestroy() {
        ...
        if (mBC85control != null) { mBC85control.destroy(); }
        mBC85control = null;
    ...
    public void ViewResult() {
        if (viewdata == false) {
            ...
            if (datos.get_status_descarga() == false) {
                Intent in = new Intent(this, view_failBC85.class);
                startActivity(in);
            }
            else {
                Intent in = new Intent(this, view_dataBC85.class);
                startActivity(in);
            }
