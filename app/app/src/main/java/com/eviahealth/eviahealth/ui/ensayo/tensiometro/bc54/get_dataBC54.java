package com.eviahealth.eviahealth.ui.ensayo.tensiometro.bc54;
...
import com.eviahealth.eviahealth.bluetooth.beurer.bc54.bc54control;
import com.eviahealth.eviahealth.models.beurer.bc54.bc54DataTensiometro;
...
public class get_dataBC54 extends BaseActivity implements View.OnClickListener {

    private String TAG = "GET_DATA_BC54";
    private String FASE = "GET_DATA_BC54";
    ...
    private String mDeviceAddress = ""; // bc54 (A4:C1:38:CC:05:4F
    private bc54control mBC54control = new bc54control();
    bc54DataTensiometro datos;
    private int retries;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_get_data_bc54);

        //region :: Views
        txtStatus = findViewById(R.id.txtStatus_bc54);
        txtStatus.setTextSize(14);
        txtStatus.setVisibility(View.VISIBLE);
        circulodescarga = findViewById(R.id.cdescarga_bc54);
        circulodescarga.setVisibility(View.INVISIBLE);

        btSaltar = findViewById(R.id.btSaltar_bc54);
        ...
        if (DEVICE_MAC_ADDRESS.contains("BC54")){
            DEVICE_MAC_ADDRESS = DEVICE_MAC_ADDRESS.replace("BC54-", "");
            mDeviceAddress = util.MontarMAC(DEVICE_MAC_ADDRESS);
            EVLog.log(TAG,"MAC mDeviceAddress: "+ mDeviceAddress);
        }

        datos = bc54DataTensiometro.getInstance();
        datos.clear();
        retries = 0;
...
//                        btSaltar.setVisibility(View.VISIBLE);
                        ...
                        mBC54control.initialize(get_dataBC54.this,mDeviceAddress);
                    }
                });
            }
        };
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch(action) {
                ...
                case BeurerReferences.ACTION_BEURER_DISCONNECTED:
                    ...
                    if (isconnect == Global.STATE_CONNECTING) {
                        if (retries < 10) {
                            ...
                            mBC54control.destroy();
                            SystemClock.sleep(1000);
                            mBC54control.initialize(get_dataBC54.this, mDeviceAddress);
                        }
                        else {
                            datos.set_status_descarga(false);
                            datos.setError("{\"type\":\"BC54\",\"error\":801,\"description\":\"No detectado dispositivo.\"}");
                            ViewResult();
                        }
                    }
                    else if (isconnect == Global.STATE_CONNECTED) {
                        // EL DISPOSITIVO SE HA DESCONECTADO SOLO
                        message = mBC54control.getRecordData();
                        ...
                        if (status == 0 ||status == 4 ) {
                            ...
                            if (status == null) {
                                String error = "{\"type\":\"BC54\",\"error\":-1,\"description\":\"No detectado dispositivo.\"}";
                                broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                            } else {
                                String error = mBC54control.msgGetMeasurementStatus(message);
                                Log.e(TAG, "Problems: " + error);

                                if (util.getIntValueJSON(error, "bodyMovement") == 1) {
                                    error = "{\"type\":\"BC54\",\"error\":1,\"description\":\"Movimiento del cuerpo.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "cuffFit") == 1) {
                                    error = "{\"type\":\"BC54\",\"error\":2,\"description\":\"El manguito está demasiado suelto.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "measurementPosition") == 1) {
                                    error = "{\"type\":\"BC54\",\"error\":6,\"description\":\"Detectada posición incorrecta del dispositivo.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 1) {
                                    error = "{\"type\":\"BC54\",\"error\":3,\"description\":\"El pulso detectado excede el limite superior.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 2) {
                                    error = "{\"type\":\"BC54\",\"error\":4,\"description\":\"El pulso detectado se encuentra por debajo del limite inferior.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else if (util.getIntValueJSON(error, "pulseRateRange") == 3) {
                                    error = "{\"type\":\"BC54\",\"error\":5,\"description\":\"Detectado un problema con la medición del pulso.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                                else {
                                    error = "{\"type\":\"BC54\",\"error\":-1,\"description\":\"not found.\"}";
                                    broadcastUpdate(BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR, error);
                                }
                            }
                        }
                ...
                case BeurerReferences.ACTION_BEURER_BC54_DOWNLOAD_DATA:
                    Log.e(TAG,"ACTION_BEURER_BC54_DOWNLOAD_DATA");
                    setTextSatus("Download");
                    break;

                case BeurerReferences.ACTION_BEURER_BC54_DOWNLOAD_DATA_FINISHED:
                    Log.e(TAG,"ACTION_BEURER_BC54_DOWNLOAD_DATA_FINISHED Message:" + message);
                ...
                case BeurerReferences.ACTION_BEURER_BC54_MEASUREMENT_ERROR:
                    Log.e(TAG,"ACTION_BEURER_BC54_MEASUREMENT_ERROR Message:" + message);
                    datos.set_status_descarga(false);
                ...
                case BeurerReferences.ACTION_BEURER_CMD_FAIL:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BC54\",\"error\":803,\"description\":\"Detectado un fallo de comunicación con el dispositivo..\"}");

                    mBC54control.disconnect();
                    break;
                ...
                case BeurerReferences.ACTION_BEURER_COMMUNICATION_TIMEOUT:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BC54\",\"error\":802,\"description\":\"Ha fallado la descarga de datos con el dispositivo.\"}");
                    mBC54control.disconnect();
                    break;
                ...
                default:
                    ...
                    datos.set_status_descarga(false);
                    datos.setError("{\"type\":\"BC54\",\"error\":804,\"description\":\"ACCION NO CONTEMPLADA\"}");
                    mBC54control.disconnect();
                    break;
    ...
    public void onDestroy() {
        ...
        if (mBC54control != null) { mBC54control.destroy(); }
        mBC54control = null;
    ...
    public void ViewResult() {
        if (viewdata == false) {
            ...
            if (datos.get_status_descarga() == false) {
                Intent in = new Intent(this, view_failBC54.class);
                startActivity(in);
            }
            else {
                Intent in = new Intent(this, view_dataBC54.class);
                startActivity(in);
            }
